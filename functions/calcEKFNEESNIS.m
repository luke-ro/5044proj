...
load("P1_vars.mat","bigF","bigC","u_nom","v_nom","nom_lmks_visible","X_nomObs_N","pos_lmks_N")
...
%     delta_X0_rand = zeros(6,1);
    ...
    [~, ~, X_simObs_N, t_obs] = simNLdynamics(w_tilde, X0_nom+X0_rand, const);
...
%     [delta_x_plus, P_plus, NEES, NIS]= LKF(delta_X0_guess, P0, Y_delta, nom_lmks_visible, bigF, Qkf, OMEGA, bigC, R, deltaX_true);
    ...
    for j=1:6
        subplot(6,1,j)
        hold on
        plot(t_obs,X_simObs_N(j,:)-X_plus(j,:),'Color',[0.5 0.5 0.5],'LineWidth',1)
        plot(t_obs,2*sqrt(squeeze(P_plus(j,j,:))'),'m','LineWidth',1)
        plot(t_obs,-2*sqrt(squeeze(P_plus(j,j,:))'),'m','LineWidth',1)
